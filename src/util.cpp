...
const char * const BITCOIN_CONF_FILENAME = "keyboardwarriorcoin.conf";
const char * const BITCOIN_PID_FILENAME = "keyboardwarriorcoin.pid";
...
#else
    const char* pszModule = "keyboardwarriorcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KeyboardWarriorCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KeyboardWarriorCoin";
#else
    // Unix
    return pathRet / ".keyboardwarriorcoin";
#endif
#endif
}
