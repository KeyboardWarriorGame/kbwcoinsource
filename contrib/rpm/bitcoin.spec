...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 58891
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 58888
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 58892
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 58891
	%{_sbindir}/semanage port -d -p tcp 58888
	%{_sbindir}/semanage port -d -p tcp 58892
